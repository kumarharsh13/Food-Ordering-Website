...
app.get("/homepage", (req,res) => {
  ...
  connection.query('SELECT user_id, user_name FROM users WHERE user_email = ? && user_name = ?', [sid,suname], function (error, results) {
    if(!error) {
...
app.get("/adminHomepage",(req,res) => {
  const sid = req.cookies.cookuid;
  const suname = req.cookies.cookuname;
  connection.query('SELECT admin_id, admin_name FROM admin WHERE admin_email = ? && admin_name = ?', [sid,suname], function (error, results) {
    if(!error) {
